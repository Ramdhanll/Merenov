...
@section('title', 'Membangun Kostan')

@section('content')
    <section class="create-rumah m-auto p-4">
      <h2>Membangun Kostan</h2>
      <p>
        Membangun kostan anda adalah kepuasan tersendiri bagi kami Mitra Merenov sebagai
        penyedia jasa bangun kostan selalu memberikan solusi yang inovatif,
        silahkan isi form dibawah ini
      </p>
      <form action="{{ route('service.store')}}" method="post">
        @csrf
        <div class="form-group">
          <input type="hidden" name="pelayanan" class="form-control text-dark @error('pelayanan') is-invalid @enderror" id="pelayanan" value="Membangun {{ucfirst($service)}}">
        </div>
        <div class="form-group">
